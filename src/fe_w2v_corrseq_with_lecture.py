...
INPUT_DIR = '../input'
NUM_WORKER = 18
MODEL_NAME = 'Word2vec_corrseq_withlec'
...
if __name__ == "__main__":
    ...
    for uid, udf in tqdm(train.groupby('user_id')):
        ...
        lseq = []
        conbined_seq = []
        for cti, uid, cid, part, tags in udf[udf.answered_correctly != 1][['content_type_id', 'user_id', 'content_id', 'part', 'tags']].values:
            if cti == 1:
                lseq.append(f'l{cid}')
        ...
        lseq = []
        conbined_seq = []
        for cti, uid, cid, part, tags in udf[udf.answered_correctly != 0][['content_type_id', 'user_id', 'content_id', 'part', 'tags']].values:
            if cti == 1:
                lseq.append(f'l{cid}')
                conbined_seq.append(f'l{cid}')
            else:
